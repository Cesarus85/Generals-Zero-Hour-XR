...
for name in TacticalGroup GroupSize Communicator SetLanguage LanguageStatus; do
  sed -n -E "/^(void|int|std::string) XrGameBoot_${name}\(/,/^}/p" GeneralsMD/Code/Main/XrGameBoot.cpp
