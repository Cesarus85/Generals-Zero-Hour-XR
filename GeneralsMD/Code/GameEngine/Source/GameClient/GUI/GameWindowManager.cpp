...
//-------------------------------------------------------------------------------------------------
// GeneralsX @feature Codex 14/09/2026 Share native tooltip hit testing with XR.
// Keep modal/Z-order ownership, including disabled and non-input widgets.
GameWindow *GameWindowManager::getWindowForTooltipAt(Int x, Int y)
{
	if(m_mouseCaptor)return m_mouseCaptor->winPointInAnyChild(x,y,TRUE,TRUE);
	if(m_grabWindow)return m_grabWindow->winPointInAnyChild(x,y,TRUE,TRUE);
	if(m_modalHead && m_modalHead->window)
		return m_modalHead->window->winPointInAnyChild(x,y,TRUE,TRUE);
	ICoord2D point;point.x=x;point.y=y;
	GameWindow *tooltip=nullptr;
	GameWindow *input=findInputTargetWindow(&point,tooltip);
	return tooltip ? tooltip:input;
}
...
{
	// GeneralsX @feature Codex 13/09/2026 UI on another surface cannot occlude world picking.
	if(TheMouse && TheMouse->getPointerTarget()==Mouse::PointerTarget::WORLD && !m_modalHead) return nullptr;
	if( m_mouseCaptor )
	{
