...
// GeneralsX @bugfix BenderAI 14/03/2026 SDL3 mouse creation must fail fast instead of returning nullptr.
	...
	// GeneralsX @feature XR port Phase 1.0 - the XR boot has no window by
	// design; SDL3Mouse only stores the handle (capture paths null-check
	// it) and all input arrives as message-stream events, so a null
	// window is a working mouse here, not a fatal error.
	if (sdlEngine && GX_XR_OffscreenBoot) {
		fprintf(stderr, "INFO: W3DGameClient::createMouse() XR offscreen mode - SDL3Mouse with null window\n");
		return NEW SDL3Mouse(nullptr);
	}
