...
void SDL3Mouse::createStreamMessages()
{
	...
	if (GX_XR_OffscreenBoot && sawRealMouse()) Mouse::createStreamMessages();
#endif
}
#endif
...
void SDL3Mouse::addSDLEvent(SDL_Event *event)
{
	...
	// GeneralsX @feature Quest port: anything arriving here with a real (non
	// touch-synthesized) source id proves a pointer device exists -- Quest
	// controller laser, OTG/Bluetooth mouse, desktop mouse. Touch platforms
	// gate pointer-defined modes (screen-edge scroll) on this instead of the
	// platform. Mirrors the SDL_TOUCH_MOUSEID drop in the poll dispatch.
	UnsignedInt which = 0;
	switch (event->type) {
