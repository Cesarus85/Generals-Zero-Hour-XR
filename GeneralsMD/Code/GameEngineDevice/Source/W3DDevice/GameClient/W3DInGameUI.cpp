...
bool W3DInGameUI::rotateXrPlacement(float radians)
{
	...
	if(!GX_XR_OffscreenBoot || !m_pendingPlaceType || !m_placeIcon[0] ||
		isPlacementAnchored() || !TheBuildAssistant ||
		TheBuildAssistant->isLineBuildTemplate(m_pendingPlaceType) || !std::isfinite(radians))return false;
	if(radians!=0) {
		const float angle=m_placeIcon[0]->getOrientation()+radians;
		m_placeIcon[0]->setOrientation(std::remainder(angle,6.283185307f));
	}
	return true;
}
#endif
...
void W3DInGameUI::drawAttackHints( View *view )
{
	...
	const float radius=18.0f+elapsed*.15f;
	for(int i=0;i<16;++i) {
		const float a=i*6.2831853f/16,b=(i+1)*6.2831853f/16;
		Vector3 start(m_xrAttackPosition.x+cosf(a)*radius,m_xrAttackPosition.y+sinf(a)*radius,0);
		Vector3 end(m_xrAttackPosition.x+cosf(b)*radius,m_xrAttackPosition.y+sinf(b)*radius,0);
		start.Z=TheTerrainLogic->getGroundHeight(start.X,start.Y)+1.5f;
		end.Z=TheTerrainLogic->getGroundHeight(end.X,end.Y)+1.5f;
		if(!m_xrAttackRing[i]) {m_xrAttackRing[i]=new Line3DClass(start,end,1.5f,1,.14f,.05f);
			W3DDisplay::m_3DScene->Add_Render_Object(m_xrAttackRing[i]);}
		else m_xrAttackRing[i]->Reset(start,end,1.5f);
		m_xrAttackRing[i]->Set_Opacity(1-elapsed/36.0f);
	}
#endif
}
...
void W3DInGameUI::clearXrAttackHint() {
	...
	m_xrAttackFrame=-1000;
}
void W3DInGameUI::createAttackHint(const GameMessage *msg) {
	extern bool GX_XR_WorldRequested();
	if(!GX_XR_WorldRequested() || !msg || !TheGameLogic) return;
	auto *target=TheGameLogic->findObjectByID(msg->getArgument(0)->objectID);
	if(target) {m_xrAttackPosition=*target->getPosition();m_xrAttackFrame=TheGameClient->getFrame();}
}
void W3DInGameUI::createForceAttackHint(const GameMessage *msg) {
	extern bool GX_XR_WorldRequested();
	if(!GX_XR_WorldRequested() || !msg) return;
	if(msg->getType()==GameMessage::MSG_DO_FORCE_ATTACK_GROUND) {
		m_xrAttackPosition=msg->getArgument(0)->location;m_xrAttackFrame=TheGameClient->getFrame();
	} else createAttackHint(msg);
}
#endif
