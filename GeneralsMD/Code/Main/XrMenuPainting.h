...
static void updateMenuTextures(XrHello &x,XrTime time) {
	...
	if(x.menu.open) {
		...
		// GeneralsX @feature Codex 23/09/2026 Paint the focused name/IP keyboard
		// from the same table that owns its ray hit regions.
		if(x.menu.page==7) {
			...
			std::string key=title+detail+xrControlsKey(table,count);
			std::string joined;
			for(const auto &text:labels) {key+='\x1f';key+=text;if(!joined.empty())joined+='\n';joined+=text;}
