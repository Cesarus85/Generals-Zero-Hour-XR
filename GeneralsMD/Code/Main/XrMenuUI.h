...
static bool updateXrMenu(XrHello &x,const XrControllerState &c,const XrView *views,XrTime time) {
	...
	if(tracked && x.panelLatched) for(int piece=0;piece<(x.menu.open ? 2:1);++piece) {
		...
		if(distance<nearest) {nearest=distance;endpoint=point;hit=piece==0 ? 100:
			(x.menu.page==7 ? ([&] {XrPanelControl table[80];const int count=xrTextKeyboardLayout(x.keyboardField==2,table,80);return xrPanelHit(table,count,u,v,kXrMenuHeight);}()) :
	...
	if(fire && hit==100) {
		if(x.menu.open && x.menu.page==7) {
			x.menu.open=false;x.keyboardField=0;x.keyboardReady=false;x.controlsArmed=false;return true;
		}
