...
{
public:
	// GeneralsX @feature Codex 13/09/2026 Independent world/window pointer surfaces.
	// Ordinary platforms keep ALL; XR switches only after draining old releases.
	enum class PointerTarget { ALL, WORLD, WINDOWS };
	PointerTarget getPointerTarget() const { return m_pointerTarget; }
	void setPointerTarget(PointerTarget target) { m_pointerTarget=target; }
private:
	PointerTarget m_pointerTarget=PointerTarget::ALL;
...
public:
	...
	// GeneralsX @feature Quest port: sticky proof that a real pointer device
	// exists on this run. Set by the device layer on the first mouse event
	// that is NOT touch-synthesized (SDL_TOUCH_MOUSEID) -- i.e. a Quest
	// controller laser, an OTG/Bluetooth mouse, or any desktop mouse. Touch
	// platforms consult this instead of the platform to decide whether
	// pointer-defined modes (screen-edge scroll) have a legitimate source.
	void markRealMouseSeen(); ///< record that a real (non-synthesized) mouse event arrived
	Bool sawRealMouse() const; ///< true once markRealMouseSeen() has run this session

	// GeneralsX @feature Quest port: true while a touch selection box is open
	// (finger down, box being drawn). Screen-edge scrolling is legitimate in
	// that state -- the pointer is defined and lifting the finger ends the
	// mode -- so touch platforms allow it despite having no hover stream.
	// Derived once per frame by the device layer, not set at transitions.
	void setTouchSelecting( Bool selecting ); ///< update the selection-box-open state
	Bool touchSelecting() const; ///< true while a touch selection box is being drawn
...
protected:
	...
	Bool m_sawRealMouse;	// set once a non-touch-synthesized mouse event arrives (see markRealMouseSeen)
	Bool m_touchSelecting;	// true while a touch selection box is open (derived per frame)
