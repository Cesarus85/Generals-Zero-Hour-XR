...
GameMessageDisposition WindowTranslator::translateGameMessage(const GameMessage *msg)
{
	...
	// GeneralsX @feature Codex 13/09/2026 Route raw pointers exclusively; keys stay shared.
	const bool rawMouse=msg->getType()>GameMessage::MSG_RAW_MOUSE_BEGIN && msg->getType()<GameMessage::MSG_RAW_MOUSE_END;
	const auto target=TheMouse ? TheMouse->getPointerTarget() : Mouse::PointerTarget::ALL;
	if(rawMouse && target==Mouse::PointerTarget::WORLD) {
		if((TheShell && TheShell->isShellActive()) || (TheWindowManager && TheWindowManager->hasModalWindow()) ||
		   (TheInGameUI && !TheInGameUI->getInputEnabled())) return DESTROY_MESSAGE;
		return KEEP_MESSAGE;
	}

	if (!(rawMouse && target==Mouse::PointerTarget::WINDOWS) && TheTacticalView && TheTacticalView->isMouseLocked())
	{
		//Kris: Aug 15, 2003
	...
	// An unused click on the detached UI must never become a world order.
	return rawMouse && target==Mouse::PointerTarget::WINDOWS ? DESTROY_MESSAGE : disp;
