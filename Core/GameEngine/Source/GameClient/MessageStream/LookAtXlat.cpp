...
// GeneralsX @feature Quest port: wider edge margins while a touch selection
// box is open. 3px is unhittable with a jittering controller laser; 48px to
// engage and 64px to release (hysteresis so jitter at the boundary does not
// flap the latch). Logical pixels at the game's render resolution.
static const Int touchBoxScrollEnterMargin = 48;
static const Int touchBoxScrollExitMargin = 64;

static Bool touchBoxScrollActive()
{
	return TheMouse != nullptr && TheMouse->touchSelecting();
}

static Int edgeScrollEnterMargin()
{
	return touchBoxScrollActive() ? touchBoxScrollEnterMargin : edgeScrollSize;
}

static Int edgeScrollExitMargin()
{
	return touchBoxScrollActive() ? touchBoxScrollExitMargin : edgeScrollSize;
}
...
Bool LookAtTranslator::canScrollAtScreenEdge() const
{
	...
	//
	// GeneralsX @feature Quest port: that day is here. A Quest controller laser (or an
	// attached mouse) IS a resting-capable pointer stream with a natural exit -- move
	// back inside, or leave the window (SDL_EVENT_WINDOW_MOUSE_LEAVE cancels the
	// scroll in the mobile lifecycle watcher). SDL3Mouse marks it on the first
	// non-touch-synthesized SDL mouse event, so this consults presence, not platform.
	// No cursor-capture requirement on mobile: grabbing would trap the Quest laser.
	//
	// An open touch selection box is the second legitimate source: the pointer is
	// defined (finger down, box drawing) and lifting the finger ends the mode, so
	// dragging the box toward an edge scrolls the map underneath it -- the
	// touch equivalent of extending a PC selection across the screen edge.
	if (TheMouse == nullptr || (!TheMouse->sawRealMouse() && !TheMouse->touchSelecting()))
		return false;
	// Fall through to the windowed/fullscreen preference checks below.
#else
	...
	switch (t)
	{
		...
		case GameMessage::MSG_RAW_MOUSE_POSITION:
		{
			...
			if (canScrollAtScreenEdge())
			{
				if (m_isScrolling)
				{
					if ( m_scrollType == SCROLL_SCREENEDGE && (m_currentPos.x >= edgeScrollExitMargin() && m_currentPos.y >= edgeScrollExitMargin() && m_currentPos.y < height-edgeScrollExitMargin() && m_currentPos.x < width-edgeScrollExitMargin()) )
					{
						stopScrolling();
					}
				}
				...
				{
					if ( m_currentPos.x < edgeScrollEnterMargin() || m_currentPos.y < edgeScrollEnterMargin() || m_currentPos.y >= height-edgeScrollEnterMargin() || m_currentPos.x >= width-edgeScrollEnterMargin() )
					{
				...
				case SCROLL_SCREENEDGE:
					{
						UnsignedInt height = TheDisplay->getHeight();
						UnsignedInt width  = TheDisplay->getWidth();
						if (m_currentPos.y < edgeScrollExitMargin())
						{
							offset.y -= TheGlobalData->m_verticalScrollSpeedFactor * fpsRatio * SCROLL_AMT * TheGlobalData->m_keyboardScrollFactor;
						}
						if (m_currentPos.y >= height-edgeScrollExitMargin())
						{
							offset.y += TheGlobalData->m_verticalScrollSpeedFactor * fpsRatio * SCROLL_AMT * TheGlobalData->m_keyboardScrollFactor;
						}
						if (m_currentPos.x < edgeScrollExitMargin())
						{
							offset.x -= TheGlobalData->m_horizontalScrollSpeedFactor * fpsRatio * SCROLL_AMT * TheGlobalData->m_keyboardScrollFactor;
						}
						if (m_currentPos.x >= width-edgeScrollExitMargin())
						{
							offset.x += TheGlobalData->m_horizontalScrollSpeedFactor * fpsRatio * SCROLL_AMT * TheGlobalData->m_keyboardScrollFactor;
						}
					}
