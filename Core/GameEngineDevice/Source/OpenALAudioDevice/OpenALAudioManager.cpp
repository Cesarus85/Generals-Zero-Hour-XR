...
//-------------------------------------------------------------------------------------------------
// GeneralsX @bugfix Codex 14/09/2026 Stop/detach before returning the cache lease.
// A stopped OpenAL source still owns AL_BUFFER. Loading the next portion may
// evict that buffer immediately; closing first caused AL_INVALID_OPERATION.
void OpenALAudioManager::releaseSampleBuffer(PlayingAudio* sample)
{
	if (sample->m_bufferHandle == 0) return;
	if (sample->m_source) {
		alSourceStop(sample->m_source);
		alSourcei(sample->m_source, AL_BUFFER, 0);
	}
	const ALuint buffer = sample->m_bufferHandle;
	sample->m_bufferHandle = 0;
	closeBuffer(buffer);
}

Bool OpenALAudioManager::startNextLoop(PlayingAudio* looping)
{
	releaseSampleBuffer(looping);
