...
ALuint OpenALAudioFileCache::getBufferForFile(const OpenFileInfo &fileInfo)
{
	...
	// GeneralsX @bugfix Codex 14/09/2026 A cache miss also returns one lease.
	// Starting at zero made a first release underflow and a playing sample
	// appear evictable before that release. Keep failed loads unleased above.
...
void OpenALAudioFileCache::closeBuffer(ALuint bufferToClose)
{
	...
	for (it = m_openFiles.begin(); it != m_openFiles.end(); ++it) {
		if (it->second.m_buffer == bufferToClose) {
			// GeneralsX @bugfix Codex 14/09/2026 Never wrap an unsigned lease count.
			if (it->second.m_openCount > 0) --it->second.m_openCount;
			return;
		}
	}
}
