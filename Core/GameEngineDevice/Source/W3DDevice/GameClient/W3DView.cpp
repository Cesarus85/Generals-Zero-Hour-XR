...
void W3DView::getPickRay(const ICoord2D *screen, Vector3 *rayStart, Vector3 *rayEnd)
{
	...
	if(GX_XR_PointerRay(screen,rayStart,rayEnd)) return;
#endif
...
void W3DView::draw()
{
	...
	if (!skipRender)
	{
		...
		W3DDisplay::m_3DScene->doRender( renderCamera );
...
Drawable *W3DView::pickDrawable( const ICoord2D *screen, Bool forceAttack, PickType pickType )
{
	...
	// GeneralsX @bugfix Codex 14/09/2026 A spatial pixel is only a token.
...
#endif
	if (TheWindowManager && !spatialPointer)
	...
	if(spatialPointer && TheTerrainRenderObject) {
		// Match the visible laser's terrain occlusion instead of picking a model
		// through a hill farther along the original bounded segment.
		CastResultStruct terrainResult;terrainResult.ComputeContactPoint=true;
		RayCollisionTestClass terrainRay(lineseg,&terrainResult);
		if(TheTerrainRenderObject->Cast_Ray(terrainRay)) {
			Vector3 epsilon=rayEnd-rayStart;epsilon.Normalize();epsilon*=.01f;
			lineseg.Set(rayStart,terrainResult.ContactPoint+epsilon);
		}
	}
...
Bool W3DView::screenToTerrain( const ICoord2D *screen, Coord3D *world )
{
	...
	for (int i = spatialPointer ? -1:int(m_locationRequests.size()) - 1; i >= 0; --i) {
	...
	req.first = (*screen);
	req.second = (*world);
	// XR can change its ray while retaining the same projected screen pixel.
	if(!spatialPointer) m_locationRequests.push_back(req);
