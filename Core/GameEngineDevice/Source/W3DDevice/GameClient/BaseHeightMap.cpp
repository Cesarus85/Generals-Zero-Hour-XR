...
bool BaseHeightMapRenderObjClass::Cast_Ray(RayCollisionTestClass & raytest)
{
	...
	lineseg=raytest.Ray;
	// GeneralsX @bugfix Codex 13/09/2026 A tracked pointer can start inside
	// the terrain's broad bounding box while still above the actual ground.
	// StartBad means "inside this box", not "no terrain intersection".
	// Keep inside endpoints and compute their cell range even on pass zero.
	P0=lineseg.Get_P0();
	P1=lineseg.Get_P1();
	...
	for (p=0; p<3; p++) {
		...
		// Even an unchanged (inside) endpoint needs the first cell bounds.
		if (!newP0 || !newP1)
			break;
