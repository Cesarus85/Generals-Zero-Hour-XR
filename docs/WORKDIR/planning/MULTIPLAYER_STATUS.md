...
- Quest can reach the LAN lobby, use Direct Connect, join/host a match and load the map. Merged PR #41 integrates Meta's runtime-owned OpenXR virtual keyboard for every focused original entry gadget, including LAN name/chat and Direct Connect fields; it does not enable or claim multiplayer compatibility and contains no LAN diagnostic or simulation changes. The maintainer accepted readable keys, controller ray, text confirmation and return to gameplay on candidate 10233; the exact final 1.2.33 APK is installed and published.
